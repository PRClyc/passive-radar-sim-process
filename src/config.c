...
struct array* array_loader(char* arg) {
    printf("trying to load antenna definition library: %s\n", arg);

    dlhandle = dlopen(arg, RTLD_NOW);
    if (!dlhandle) {
        fprintf(stderr, "dlopen failed: %s\n", dlerror());
        return NULL;
    }

    struct array* (*create_array_model)(void);
    create_array_model = dlsym(dlhandle, "create_array_model");

    if (!create_array_model) {
        fprintf(stderr, "dlsym failed: %s\n", dlerror());
        dlclose(dlhandle);
        return NULL;
    }

    struct array* arr = create_array_model();
    if (!arr) {
    ...
    printf("successfully loaded antenna lib!\n");
